...
/*
 *	Ex 3) Altere a função anterior para mostrar quantos nós foram percorridos até encontrar o valor.
 */
...
int buscarValor(No *raiz, int valor){
	...
	if(!raiz){
		...
		qtd += buscarValor(raiz->esq, valor);
	}else if(raiz->valor < valor){
		qtd += buscarValor(raiz->dir, valor);
	}
	qtd++;
	return qtd;
}
